...
import { Server, Activity, ArrowDown, ArrowUp, Shield, ShieldAlert } from 'lucide-react';
...
const NetworkTopology = () => {
  ...
  const API_URL = 'http://35.213.177.111:8080/api/network/topology';
  ...
  const fetchTopologyData = async () => {
    try {
      ...
      console.error("Lỗi khi lấy dữ liệu Topology:", error);
    }
  };

  useEffect(() => {
    fetchTopologyData(); 
    const interval = setInterval(fetchTopologyData, 3000);
  ...
  const NodeCard = ({ title, ip, role, data, color, isVictim }) => (
    <div style={{
      width: '260px',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <Server size={24} color={color} style={{ marginRight: '10px' }} />
          <div>
            <h3 style={{ margin: 0, fontSize: '16px', color: '#1f2937' }}>{title}</h3>
            <span style={{ fontSize: '12px', color: '#6b7280' }}>{ip}</span>
          </div>
        </div>
        {/* Hiển thị Iptables cho VM1 */}
        {isVictim && (
          isUnderAttack ? 
            <ShieldAlert size={24} color="#ef4444" className="animate-pulse" /> : 
      ...
      {data && data.status !== "OFFLINE" ? (
        ...
        <div style={{ fontSize: '13px', color: '#9ca3af', textAlign: 'center', padding: '20px 0' }}>Đang chờ dữ liệu...</div>
      )}
    </div>
  );

  return (
    <div style={{ padding: '20px', minHeight: '75vh', backgroundColor: '#f9fafb' }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '30px' }}>
        <Activity size={28} color="#4f46e5" style={{ marginRight: '10px' }} />
        <h2 style={{ fontSize: '24px', color: '#1f2937', margin: 0 }}>Luồng Gói Tin & Giám Sát Topology</h2>
      </div>
