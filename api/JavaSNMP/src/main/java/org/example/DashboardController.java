// DashboardController.java: Endpoint API điều khiển chính của hệ thống NMS
package org.example;
...
public class DashboardController {
    ...
    // Cung cấp API GET để Frontend theo dõi tức thời hiệu năng (CPU, RAM, Tốc độ mạng) của các Agents.
    @GetMapping("/metrics")
    public ResponseEntity<Map<String, Map<String, Object>>> getAllMetrics() {
        return ResponseEntity.ok(pollerService.getAllMetrics());
    }

    // Cung cấp API POST cho phép Frontend thêm một Agent IP mới vào mạng lưới giám sát.
    ...
    // Cung cấp API GET để hiển thị các thông báo (TRAP Alert) nếu máy Agent đang bị tấn công.
    @GetMapping("/alerts")
    public ResponseEntity<List<Map<String, String>>> getAlerts() {
        return ResponseEntity.ok(trapService.getAlertHistory());
    }

    // Cung cấp API POST điều khiển từ xa, kích hoạt lệnh Iptables để chặn cuộc tấn công trên Agent mục tiêu.
    ...
    public ResponseEntity<String> activateShield(@RequestParam String targetIp,
        ...
        boolean success = trapService.triggerMitigationSet(targetIp, typeId);
        if (success) {
            return ResponseEntity.ok("Shield activated successfully (Type " + typeId + ") on " + targetIp);
        } else {
            return ResponseEntity.status(500).body("Failed to activate shield!");
        }
    }

    // Cung cấp API POST cập nhật thời gian trễ (để test hiệu năng chênh lệch khi bị tấn công).
    ...
    // Cung cấp API GET trích xuất Nhật ký Đánh giá chứa các số liệu sau quá trình Anti-DDoS.
    @GetMapping("/logs")
    public ResponseEntity<List<Map<String, Object>>> getEvaluationLogs() {
        return ResponseEntity.ok(trapService.getEvaluationLogs());
    }

    // Cung cấp API GET để kiểm tra xem IP có hỗ trợ tính năng IPS (TRAP & SET) không
